...
class MMLoss(nn.Module):
    def __init__(self, lam1=1, lam2=1, sample_n=4096, input_size=480, sample_size=16, safe_radius_neg=7,
    ...
    def compute_hard_dist(self, feat1, feat2, position):
        ...
        M = ((torch.pi - neg_k.acos()).pow(2) / 3 + (torch.pi - neg_j.acos()).pow(2) / 3 + (
                    torch.pi - neg_cross.acos()).pow(2) / 3 + \
    ...
    def loss_desc(self):
        ...
        for i in range(self.score1.shape[0]):
            ...
            device = self.score1.device
            feat1, score1, feat2, score2, position = self.ramdom_sampler(self.feat1[i], self.AP3(self.score1[i]),
                                                                         self.feat2_warped[i],
                                                                         self.AP3(self.score2_warped[i]),
                                                                         self.good_mask[i])
            ...
            # if torch.isnan(loss):
            #     loss = torch.tensor(0.0, device=device)  # 将 NaN 值替换为零
            assert not loss.isnan()
        return loss / (i + 1)

    def loss_rep(self):
        feat_wise_point_simi = ((self.feat1 * self.feat2_warped).sum(dim=1, keepdim=True)).detach() * self.good_mask
        good_mask = F.unfold(feat_wise_point_simi, kernel_size=self.sample_size, padding=0,
                             stride=self.sample_size // 2).transpose(1, 2)
        patches1 = F.unfold(self.score1 * self.good_mask, kernel_size=self.sample_size, padding=0,
                            stride=self.sample_size // 2).transpose(1, 2)
        patches1 = F.normalize(patches1, dim=2)
        patches2 = F.unfold(self.score2_warped * self.good_mask, kernel_size=self.sample_size, padding=0,
                            stride=self.sample_size // 2).transpose(1, 2)
        patches2 = F.normalize(patches2, dim=2)
        patches_simi = F.unfold(feat_wise_point_simi, kernel_size=self.sample_size, padding=0,
                                stride=self.sample_size // 2).transpose(1, 2)
    ...
    def loss_peak(self):
        ...
        loss_peak_edge = (mask1 * score1.pow(2)).mean() / self.mask1_mean + (
                    mask2 * score2.pow(2)).mean() / self.mask2_mean

        loss_peak_random = self.AP(score1_).pow(2).mean() + (1 - self.MP(score1_)).pow(2).mean() + (
                    self.AP3(score1_) + 1 - self.MP3(score1_)).pow(2).mean() + \
                           self.AP(score2_).pow(2).mean() + (1 - self.MP(score2_)).pow(2).mean() + (
                                       self.AP3(score2_) + 1 - self.MP3(score2_)).pow(2).mean()

        loss_peak_coupled = 0
        for i in range(self.score1.shape[0]):
            # if number of samples is too samll or scores are all zeros, randomly generate a new mask
            feat1, score1, feat2, score2, position = self.ramdom_sampler(self.feat1[i], self.score1[i],
                                                                         self.feat2_warped[i],
